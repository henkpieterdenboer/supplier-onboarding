  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(invitationUrl, t('emails.invitation.button'))}
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(invitationUrl, t('emails.supplierSave.button'))}
        <p><strong>${t('emails.linkValid', { date: formatDate(expiresAt, language) })}</strong></p>
        <p>${t('emails.contactInfo')}</p>
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #666; font-size: 12px;">
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(requestUrl, t('emails.purchaserNotification.button'))}
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #666; font-size: 12px;">
          ${t('emails.autoGenerated')}
        </p>
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(requestUrl, t('emails.financeNotification.button'))}
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #666; font-size: 12px;">
          ${t('emails.autoGenerated')}
        </p>
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(requestUrl, t('emails.erpNotification.button'))}
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #666; font-size: 12px;">
          ${t('emails.autoGenerated')}
        </p>
...
interface CompletionEmailOptions {
  financeRecipients: { email: string; preferredLanguage: string | null }[]
...
export async function sendCompletionEmail({
  ...
  for (const fu of financeRecipients) {
    const lang = (fu.preferredLanguage || 'nl') as Language
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(requestUrl, t('emails.completion.button'))}
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #666; font-size: 12px;">
          ${t('emails.autoGenerated')}
        </p>
  ...
  // Email to Purchaser (in purchaser's preferred language)
  if (purchaserEmail) {
    const tp = (key: string, vars?: Record<string, string | number>) => getTranslation(language, key, vars)
    await sendEmail({
      to: purchaserEmail,
      subject: tp('emails.completion.subject', { supplierName }),
      language,
      label,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">${tp('emails.completion.title')}</h2>
          <p>${tp('emails.completion.greetingPurchaser', { purchaserName })}</p>
          <p>${tp('emails.completion.bodyPurchaser', { supplierName })}</p>
          <div style="background-color: #f3f4f6; padding: 16px; border-radius: 8px; margin: 20px 0;">
            <p style="margin: 0;"><strong>${tp('emails.completion.creditorNumber')}</strong> ${creditorNumber}</p>
            <p style="margin: 8px 0 0 0;"><strong>${tp('emails.completion.kbtCode')}</strong> ${kbtCode}</p>
          </div>
          ${emailButton(requestUrl, tp('emails.completion.button'))}
          <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
          <p style="color: #666; font-size: 12px;">
            ${tp('emails.autoGenerated')}
          </p>
        </div>
      `,
    })
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(activationUrl, t('emails.activation.button'))}
        <p><strong>${t('emails.linkValid', { date: formatDate(expiresAt, language) })}</strong></p>
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(resetUrl, t('emails.passwordReset.button'))}
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        ${emailButton(actionUrl, actionText)}
        <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
        <p style="color: #666; font-size: 12px;">
          ${t('emails.autoGenerated')}
        </p>
